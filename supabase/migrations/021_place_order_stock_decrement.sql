-- Follow-up to 019_place_order_rpc.sql: that migration deliberately
-- VALIDATED requested qty against products.stock but did NOT decrement it,
-- flagging the decrement itself as a follow-up product decision (see its
-- trailing comment). This migration is that follow-up: place_order() now
-- decrements products.stock atomically with the order/order_items insert,
-- using `select ... for update` row locking on each product BEFORE
-- re-checking stock, so two concurrent checkouts racing on the same
-- product's last few units can no longer both pass a stale stock check and
-- both succeed (the exact race the original audit flagged). The second
-- transaction blocks on the row lock until the first commits or rolls back,
-- then re-reads the now-current stock.
--
-- Every other behavior of place_order() — signature, idempotency-on-
-- p_order_id, authorization checks, return shape — is preserved exactly.
-- This is `create or replace function`, not an edit to 019's file, per the
-- "never edit an already-written migration" rule.
...
-- Signature is unchanged from 019, so the existing grants already apply;
-- re-stating them here is harmless and keeps this migration self-contained.
